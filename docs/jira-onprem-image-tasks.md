...
**Description:**
image แยกที่มี **prisma CLI + schema + init migration + seed เท่านั้น (ไม่มี app source)**. รันบนเครื่องลูกค้า → `prisma migrate deploy` (สร้าง schema) + `prisma db seed` ครั้งเดียวแล้ว exit. bake init migration จาก schema ตอน build (เพราะ harness ใช้ db push ไม่มี migrations dir). idempotent + seed upsert → รันซ้ำปลอดภัย. Tag `…-migrate:v<n>`.
...
**Description:**
ตอนนี้ init migration ถูก regenerate ทุก build (CRN regen schema ใหม่ทุกครั้ง ไม่ incremental) → ถ้า demo อัปเดต (v2) + ลูกค้ามี data แล้ว, `migrate deploy` ของ v2 (checksum ต่าง) จะ **fail checksum mismatch**. ต้องทำ migration แบบ delta จริง: track schema เดิม → diff → migration ใหม่ต่อท้าย.

**Acceptance Criteria:**
- deploy v2 ทับ v1 ที่มี data → schema อัปเดต (delta) โดย **data ไม่หาย** ไม่ checksum-fail
