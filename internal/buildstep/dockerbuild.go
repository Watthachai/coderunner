...
# one FAILS LOUDLY (container won't start) instead of silently dropping data. Seed runs
# by DEFAULT (set DEMO_SEED=0 to disable) — the seed is idempotent, and an auth demo is
# unusable without its login user, so seeding out-of-the-box is the safe default.
# No-op migrate when the app has no Prisma.
RUN printf '#!/bin/sh\nset -e\nif [ -f prisma/schema.prisma ]; then\n  echo "[crn] migrate-on-start: prisma db push"\n  npx prisma db push --skip-generate\n  if [ "${DEMO_SEED:-1}" != "0" ]; then echo "[crn] seeding demo data"; npx prisma db seed || echo "[crn] seed failed (continuing)"; fi\nfi\nexec node server.js\n' > /app/docker-entrypoint.sh \
...
# (prisma db push) and seeds example data + the dev login user by default.
services:
  app:
    image: {{APP_IMAGE}}
    environment:
      DATABASE_URL: "${DATABASE_URL:?set DATABASE_URL to your Postgres, e.g. postgresql://user:pass@host:5432/db}"
      # DEMO_SEED: "0"   # uncomment to DISABLE seeding (default seeds on start)
    ports:
      - "${APP_PORT:-{{PORT}}}:3000"
`
...
2. โหลด seed data ตัวอย่าง + dev login user (รัน default; ตั้ง ` + "`DEMO_SEED=0`" + ` เพื่อปิด)
